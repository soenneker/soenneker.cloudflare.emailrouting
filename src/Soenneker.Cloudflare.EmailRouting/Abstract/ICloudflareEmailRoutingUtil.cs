...
public interface ICloudflareEmailRoutingUtil
{
    ...
    /// </summary>
    ValueTask<EmailDestinationAddressResponseSingle?> AddDestinationAddress(string accountIdentifier, string email, CancellationToken cancellationToken = default);

    /// <summary>
    /// Removes a destination address from email routing.
    /// </summary>
    ValueTask<EmailDestinationAddressResponseSingle?> RemoveDestinationAddress(string accountIdentifier, string destinationAddressId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Lists all destination addresses.
    /// </summary>
    ValueTask<EmailDestinationAddressesResponseCollection?> ListDestinationAddresses(string accountIdentifier, CancellationToken cancellationToken = default);

    /// <summary>
    /// Creates a custom email address with routing rules.
    /// </summary>
    ValueTask<EmailRuleResponseSingle?> CreateCustomAddress(string zoneIdentifier, string customEmail, string destinationEmail, CancellationToken cancellationToken = default);

    /// <summary>
    /// Removes a custom email address and its routing rules.
    /// </summary>
    ValueTask<EmailRuleResponseSingle?> RemoveCustomAddress(string zoneIdentifier, string ruleId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Lists all routing rules.
    /// </summary>
    ValueTask<EmailRulesResponseCollection?> ListRoutingRules(string zoneIdentifier, CancellationToken cancellationToken = default);
